...
training_set = importdata('data13286.txt');

labels = importdata('label13286.txt');

var=size(training_set,1);

rand_sel = randperm(var); %randomly shuffle the training examples
        seventy_per_cent = round(0.7*var);
 ...
 [m n] = size(data_set);%assign 70% of the dataset to the training set
        ...
        cv_set = training_set(rand_sel((1+seventy_per_cent):end),:); %assign the rest of 30% of the dataset to the cross-validation set
        cv_labels = labels(rand_sel((1+seventy_per_cent):end),:);
...
lambda = 0;
options = optimset('MaxIter', 150); %the more interations, the better the accuracy
...
for i=1:size(cv_set,1)
    ...
    if predictions(i)~=cv_labels(i)
        ...
        R=[R; char(convert(predictions(i))) score(i) char(convert(cv_labels(i)))];
        ...
        if (predictions(i)=='1'&& cv_labels(i)=='i' ||cv_labels(i)=='I')||((predictions(i)=='i' || predictions(i)=='I')&& cv_labels(i)=='1')
        count=count-1;
        end
            count=count+1;
    else 
